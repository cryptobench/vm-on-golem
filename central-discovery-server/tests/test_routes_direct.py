...
async def test_routes_direct_return_paths():
    ...
    res = await routes.create_advertisement(
        advertisement=adv, provider_id="p", repo=repo
    )
    assert res == {"ok": True}
    ...
    # delete success path
    res3 = await routes.delete_advertisement(
        provider_id="will-delete", current_provider="will-delete", repo=repo
    )
    assert res3 == {"status": "success"}

    # delete not found path (covers 404 raise)
    with pytest.raises(Exception):
        await routes.delete_advertisement(
            provider_id="not-there", current_provider="not-there", repo=repo
        )
