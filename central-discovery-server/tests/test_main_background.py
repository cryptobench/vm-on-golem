...
async def test_cleanup_task_branches_and_cancel(monkeypatch):
    ...
    monkeypatch.setattr(
        m.AdvertisementRepository, "cleanup_expired", fake_cleanup_ok, raising=False
    )
    ...
    monkeypatch.setattr(
        m.AdvertisementRepository, "cleanup_expired", fake_cleanup_fail, raising=False
    )
    task = asyncio.create_task(m.cleanup_expired_advertisements())
    await asyncio.sleep(0.02)
    task.cancel()
...
def test_main_guard_executes_start(monkeypatch):
    ...
    fake_uvicorn = types.SimpleNamespace(
        run=lambda *a, **k: called.update({"ok": True})
    )
