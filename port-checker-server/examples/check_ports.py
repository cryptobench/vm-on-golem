...
async def check_ports(
    ip: str, ports: List[int], server_url: str = "http://localhost:7466"
):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                f"{server_url}/check-ports", json={"provider_ip": ip, "ports": ports}
            ) as response:
                if response.status == 200:
                    ...
                    for port, status in result["results"].items():
                        ...
                        print(
                            f"{icon} Port {port}: {'Accessible' if status['accessible'] else 'Not accessible'}{error}"
                        )
