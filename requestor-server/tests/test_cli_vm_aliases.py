...
def load_commands(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules, "requestor.config", types.SimpleNamespace(config=config_stub)
    )
    monkeypatch.setitem(
        sys.modules,
        "requestor.services.database_service",
        types.SimpleNamespace(DatabaseService=DummyDatabaseService),
    )
    monkeypatch.setitem(
        sys.modules,
        "requestor.services.ssh_service",
        types.SimpleNamespace(SSHService=object),
    )
    monkeypatch.setitem(
        sys.modules,
        "requestor.services.vm_service",
        types.SimpleNamespace(VMService=object),
    )
    monkeypatch.setitem(
        sys.modules,
        "requestor.services.provider_service",
        types.SimpleNamespace(ProviderService=object),
    )
    monkeypatch.setitem(
        sys.modules,
        "requestor.provider.client",
        types.SimpleNamespace(ProviderClient=object),
    )
    monkeypatch.setitem(
        sys.modules, "requestor.errors", types.SimpleNamespace(RequestorError=Exception)
    )
...
def test_vm_connect_alias_calls_ssh_callback(runner, monkeypatch):
    ...
    monkeypatch.setattr(
        commands, "db_service", types.SimpleNamespace(init=lambda: None)
    )
