...
from ..utils.spinner import Spinner, step
...
@click.option(
    "--network",
    type=click.Choice(["development", "testnet", "mainnet"]),
    default=None,
...
def version_cmd():
    ...
    if latest and latest != current:
        ...
        click.echo(
            f"Requestor CLI: {current} (up-to-date)"
            if latest
            else f"Requestor CLI: {current}"
        )
...
@click.option(
    "--driver",
    type=click.Choice(["arkiv", "central", "golem-base"]),
    default=None,
    help="Discovery backend to use",
)
@click.option(
    "--payments-network",
    type=str,
    default=None,
    help="Filter by payments network profile (default: current config)",
)
@click.option(
    "--all-payments", is_flag=True, help="Do not filter by payments network (show all)"
)
...
async def list_providers(
    ...
    try:
        ...
        # Determine the discovery backend being used. "golem-base" is a legacy alias.
        discovery_driver = (
            driver
        ...
        eff_pn = (
            payments_network
            if payments_network is not None
            else getattr(config, "payments_network", None)
        )
        logger.process(
            f"Querying discovery via {discovery_driver} (network={config.network}, payments={eff_pn if not all_payments else 'ALL'})"
        )
...
@click.option(
    ...
    type=int,
    default=None,
    help="Optional StreamPayment stream id to fund this VM",
)
@click.option(
    "--hours",
    type=int,
    default=1,
    help="If no stream-id is provided and payments are enabled, open a stream with this many hours of deposit (default 1)",
)
...
@click.option(
    ...
    type=click.Choice(["development", "testnet", "mainnet"]),
    default=None,
    help="Override network for discovery during creation",
)
@async_command
async def create_vm(
    name: str,
    ...
    try:
        ...
        click.echo(
            f"  Resources  : {click.style(f'{cpu} CPU, {memory}GB RAM, {storage}GB Disk', fg='cyan')}"
        )
        click.echo("─" * 60 + "\n")
        ...
        with Spinner("Deploying VM..."):
            ...
            async with provider_service:
                ...
                if not await provider_service.check_resource_availability(
                    ...
                    raise RequestorError(
                        "Provider doesn't have enough resources available"
                    )
                ...
                provider_service.estimate_spec = (cpu, memory, storage)
                est_row = await provider_service.format_provider_row(
                    provider, colorize=False
                )
                ...
                click.echo(
                    click.style(
                        f"  💵 Estimated Monthly Cost: {price_str}",
                        fg="yellow",
                        bold=True,
                    )
                )
        ...
        if "Failed to query discovery service" in error_msg:
            error_msg = (
                "Unable to reach discovery service (check your internet connection)"
            )
...
async def stream_list(as_json: bool):
    ...
    try:
        ...
        if not vms:
            ...
            click.echo(
                json.dumps({"streams": []}, indent=2) if as_json else "No VMs found."
            )
            return {"streams": []}
        ...
        if as_json:
            ...
            for r in results:
                ...
                rows.append(
                    [
                        ...
                        r.get("reason")
                        or ("error: " + r.get("error") if r.get("error") else ""),
            ...
            click.echo("\n" + "─" * 60)
            click.echo(
                click.style(f"  💸 Streams ({len(results)} VMs)", fg="blue", bold=True)
            )
            click.echo("─" * 60)
            click.echo(
                "\n"
...
@click.option(
    "--hours", type=int, default=1, help="Deposit coverage in hours (default 1)"
)
@click.option(
    "--network",
    type=click.Choice(["development", "testnet", "mainnet"]),
    default=None,
    help="Override network for discovery during stream open",
)
@async_command
...
@click.option(
    "--hours", type=int, required=False, help="Hours of coverage to add at prior rate"
)
@async_command
async def stream_topup(stream_id: int, glm: float | None, hours: int | None):
    ...
    try:
        ...
        click.echo(
            json.dumps(
                {"stream_id": stream_id, "topped_up_wei": add_wei, "tx": tx}, indent=2
            )
        )
    except Exception as e:
...
async def stream_status(name: str, as_json: bool):
    ...
    try:
        ...
        click.echo("\n" + "─" * 60)
        click.echo(
            click.style(f"  💸 Stream Status for VM: {name}", fg="blue", bold=True)
        )
        click.echo("─" * 60)
        click.echo(
            f"  Stream ID     : {click.style(str(status.get('stream_id')), fg='cyan')}"
        )
        click.echo(
            f"  Verified      : {click.style(str(status.get('verified')), fg='green' if status.get('verified') else 'yellow')}"
        )
        ...
        click.echo("  Computed      :")
        click.echo(
            f"    now         : {comp.get('now')}  remaining: {comp.get('remaining_seconds')}s"
        )
        click.echo(
            f"    vested      : {comp.get('vested_wei')}  withdrawable: {comp.get('withdrawable_wei')}"
        )
        click.echo("─" * 60)
    except Exception as e:
...
async def stream_inspect(stream_id: int, as_json: bool):
    ...
    try:
        ...
        if as_json:
            ...
            click.echo(
                click.style(
                    f"  🔎 On-chain Stream Inspect: {stream_id}", fg="blue", bold=True
                )
            )
            click.echo("─" * 60)
                ...
                json.dumps(
                    {
                        ...
                        "network": getattr(config, "payments_network", None),
                    },
                    indent=2,
                )
            )
    ...
    try:
        ...
        if results["success"]:
            ...
            for name, msg in results["success"]:
                click.echo(
                    f"  • {click.style(name, fg='cyan')}: {click.style(msg, fg='green')}"
                )
            click.echo()
        ...
        if results["failed"]:
            ...
            for name, error in results["failed"]:
                click.echo(
                    f"  • {click.style(name, fg='cyan')}: {click.style(error, fg='red')}"
                )
            click.echo()
        ...
        click.echo("  " + "┈" * 25)
        click.echo(
            f"  📈 Success Rate : {click.style(f'{success_rate:.1f}%', fg='cyan')}"
        )
        click.echo(
            f"  ✅ Successful   : {click.style(str(len(results['success'])), fg='green')}"
        )
        click.echo(
            f"  ❌ Failed       : {click.style(str(len(results['failed'])), fg='red')}"
        )
...
async def start_vm(name: str):
    ...
    try:
        ...
        click.echo(
            f"  🔌 Port       : {click.style(str(vm['config']['ssh_port']), fg='cyan')}"
        )
...
def run_api_server(host: str, port: int, reload: bool):
    ...
    try:
        ...
        logger.error(
            f"Failed to create database directory {config.db_path.parent}: {e}"
        )
        raise click.Abort()
...
async def list_vms(as_json: bool):
    ...
    try:
        ...
        if "database" in error_msg.lower():
            error_msg = (
                "Failed to access local database (try running the command again)"
            )
