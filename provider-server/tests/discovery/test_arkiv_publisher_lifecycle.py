...
def _expected_annotations(resources, ip):
    # Mirror publisher logic for platform
    ...
    string_annotations = [
        ...
        _Ann(
            "golem_price_usd_core_month", str(float(settings.PRICE_USD_PER_CORE_MONTH))
        ),
        _Ann(
            "golem_price_usd_ram_gb_month",
            str(float(settings.PRICE_USD_PER_GB_RAM_MONTH)),
        ),
        _Ann(
            "golem_price_usd_storage_gb_month",
            str(float(settings.PRICE_USD_PER_GB_STORAGE_MONTH)),
        ),
        _Ann(
            "golem_price_glm_core_month", str(float(settings.PRICE_GLM_PER_CORE_MONTH))
        ),
        _Ann(
            "golem_price_glm_ram_gb_month",
            str(float(settings.PRICE_GLM_PER_GB_RAM_MONTH)),
        ),
        _Ann(
            "golem_price_glm_storage_gb_month",
            str(float(settings.PRICE_GLM_PER_GB_STORAGE_MONTH)),
        ),
    ]
    numeric_annotations = [
...
async def test_create_when_no_existing(monkeypatch):
    ...
    settings.ARKIV_ADVERTISEMENT_INTERVAL = 360
...
async def test_delete_when_multiple_and_recreate(monkeypatch):
    ...
    settings.ARKIV_ADVERTISEMENT_INTERVAL = 100
    await adv.post_advertisement()
...
async def test_update_when_annotations_differ(monkeypatch):
    ...
    monkeypatch.setattr(
        arkiv_publisher,
        "get_provider_entity_keys",
        lambda *a, **k: asyncio.sleep(0, result=[key]),
    )

    # Current metadata deliberately mismatched (empty annotations)
    client.set_metadata(
        key,
        _Meta(expires_at_block=9999999, string_annotations=[], numeric_annotations=[]),
    )

    settings.ARKIV_ADVERTISEMENT_INTERVAL = 50
    await adv.post_advertisement()
...
async def test_extend_when_up_to_date_and_near_expiry(monkeypatch):
    ...
    desired_interval = 50
    settings.ARKIV_ADVERTISEMENT_INTERVAL = desired_interval
    ...
    client.set_metadata(
        ...
        _Meta(
            ...
            string_annotations=s_anns,
            numeric_annotations=n_anns,
        ),
    )
...
async def test_skip_when_resources_low(monkeypatch):
    ...
    assert (
        not client.created
        and not client.updated
        and not client.deleted
        and not client.extended
    )
