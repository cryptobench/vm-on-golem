...
class StubProviderLiveService:
    async def stream_provider(self, websocket):
        await websocket.accept()
        await websocket.send_json(
            {
                "type": "hello",
                "generated_at": "2026-05-14T18:00:00+00:00",
                "scope": None,
                "data": {"protocol": "provider-live.v1"},
                "error": None,
            }
        )


def test_provider_live_route_is_websocket_endpoint():
    client = TestClient(app)
    with app.container.provider_live_service.override(StubProviderLiveService()):
        with client.websocket_connect("/api/v1/provider/live") as websocket:
            event = websocket.receive_json()

    assert event["type"] == "hello"
    assert event["data"]["protocol"] == "provider-live.v1"
