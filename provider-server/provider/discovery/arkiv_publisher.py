...
class ArkivDiscoveryPublisher(DiscoveryPublisher):
    ...
    async def post_advertisement(self):
        ...
        try:
            existing_keys = await get_provider_entity_keys(
                self.client, settings.PROVIDER_ID
            )
            ...
            string_annotations = [
                ...
                Annotation(
                    key="golem_payments_network", value=settings.PAYMENTS_NETWORK
                ),
                Annotation(key="golem_provider_id", value=settings.PROVIDER_ID),
                Annotation(key="golem_ip_address", value=ip_address),
                Annotation(key="golem_endpoint_protocol", value=endpoint_protocol),
                Annotation(key="golem_endpoint_host", value=endpoint_host),
                Annotation(
                    key="golem_endpoint_port",
                    value=str(endpoint_port),
                ),
                Annotation(key="golem_endpoint_url", value=endpoint_url),
                ...
                Annotation(
                    key="golem_price_usd_core_month",
                    value=str(float(settings.PRICE_USD_PER_CORE_MONTH)),
                ),
                Annotation(
                    key="golem_price_usd_ram_gb_month",
                    value=str(float(settings.PRICE_USD_PER_GB_RAM_MONTH)),
                ),
                Annotation(
                    key="golem_price_usd_storage_gb_month",
                    value=str(float(settings.PRICE_USD_PER_GB_STORAGE_MONTH)),
                ),
                Annotation(
                    key="golem_price_glm_core_month",
                    value=str(float(settings.PRICE_GLM_PER_CORE_MONTH)),
                ),
                Annotation(
                    key="golem_price_glm_ram_gb_month",
                    value=str(float(settings.PRICE_GLM_PER_GB_RAM_MONTH)),
                ),
                Annotation(
                    key="golem_price_glm_storage_gb_month",
                    value=str(float(settings.PRICE_GLM_PER_GB_STORAGE_MONTH)),
                ),
            ]
            ...
            if len(existing_keys) > 1:
                logger.warning(
                    f"Found {len(existing_keys)} advertisements. Cleaning up and creating a new one."
                )
                deletes = [GolemBaseDelete(entity_key=key) for key in existing_keys]
                await self.client.delete_entities(deletes)
                await self._create_advertisement(
                    string_annotations, numeric_annotations
                )
                ...
                expected_annotations = {
                    ann.key: ann.value for ann in string_annotations
                }
                expected_annotations.update(
                    {ann.key: ann.value for ann in numeric_annotations}
                )
                ...
                desired_btl_blocks = (
                    int(settings.ARKIV_ADVERTISEMENT_INTERVAL) * 2
                )  # ~2 blocks/sec

                if sorted(current_annotations.items()) == sorted(
                    expected_annotations.items()
                ):
                    # Refresh TTL proactively if nearing expiry
                    try:
                        current_block = (
                            await self.client.http_client().eth.get_block_number()
                        )
                        remaining_blocks = int(metadata.expires_at_block) - int(
                            current_block
                        )
                    except Exception:
                    ...
                    if remaining_blocks <= refresh_threshold:
                        ...
                        ext = GolemBaseExtend(
                            entity_key=entity_key, number_of_blocks=desired_btl_blocks
                        )
