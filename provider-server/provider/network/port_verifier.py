...
class PortVerifier:
            ...
            try:
                ...
                if port == self.discovery_port:
                    try:
                        ...
                        logger.debug(
                            f"Created temporary listener for discovery port {port}"
                        )
                        continue
                    except Exception as e:
                        if (
                            isinstance(e, OSError) and e.errno == 98
                        ):  # Address already in use
                            # This might be our own server starting up
                            available_ports.add(port)
                            logger.debug(
                                f"Port {port} is already in use - this is expected if our server is starting"
                            )
                            continue
                        logger.debug(
                            f"Failed to create temporary listener for discovery port {port}: {e}"
                        )
                        continue
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    response = await session.post(
                        ...
                        json={"provider_ip": public_ip, "ports": list(ports)},
                    ...
                    if response.status == 200:
                        ...
                        for port_key, result in raw_results.items():
                            ...
                            if port not in results or (
                                accessible and not results[port].accessible
                            ):
                ...
                attempts.append(
                    ServerAttempt(server=server, success=False, error=error_msg)
                )
                logger.warning(
                    f"{error_msg}. Please ensure the port check server is running and accessible."
                )
                ...
                logger.warning(
                    f"{error_msg}. Please ensure the port check server is running."
                )
            except Exception as e:
    ...
    async def _create_temp_listener(self, port: int) -> Optional[asyncio.Server]:
        ...
        try:
            ...
            if result == 0:
                logger.debug(
                    f"Port {port} is already in use - this is expected if our server is running"
                )
                return None
    ...
    async def verify_ports(self, ports: List[int]) -> Dict[int, PortVerificationResult]:
        ...
        async for local_available in self.verify_local_binding(ports):
            ...
            accessible_ports = [
                port for port, result in results.items() if result.accessible
            ]
            if accessible_ports:
                logger.info(
                    f"Successfully verified {len(accessible_ports)} SSH ports: {', '.join(map(str, sorted(accessible_ports)))}"
                )
            else:
