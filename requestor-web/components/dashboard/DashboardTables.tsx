...
function platformLabel(platform?: string | null) {
  ...
  if (lower.includes("windows"))
    return { Icon: RiWindowsLine, label: "Windows" };
  return { Icon: RiUbuntuLine, label: value };
}

export function ActiveVmsTable({ vms }: { vms: RequestorVmModel[] }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-max border-collapse text-sm">
        ...
        <tbody>
          {vms.map((vm) => (
            <ActiveVmRow key={vm.rental.vm_id} vm={vm} />
          ))}
        </tbody>
      </table>
    </div>
...
function ActiveVmRow({ vm }: { vm: RequestorVmModel }) {
  const { rental } = vm;
  const platform = platformLabel(vm.platform);
  ...
  return (
    <tr className="border-b border-border last:border-b-0">
      ...
      <td className="px-4 py-4">
        <DashboardStatus status={vm.lifecycle.status} />
      </td>
      <td className="px-4 py-4 font-mono text-text-primary">
        {shortId(rental.provider_id)}
      </td>
      <td className="px-4 py-4 font-mono text-text-primary">
        {shortId(rental.vm_id)}
      </td>
      <td className="px-4 py-4">
      ...
      <td className="px-4 py-4">{resourceValue(vm.resources, "cpu")}</td>
      <td className="px-4 py-4">{resourceValue(vm.resources, "memory")} GB</td>
      <td className="px-4 py-4">{resourceValue(vm.resources, "storage")} GB</td>
      <td className="px-4 py-4 font-mono text-text-primary">
        {vm.sshEndpoint}
      </td>
      <td className="py-4 pl-4 text-right">
        <button
          className="rounded-md p-1 text-text-primary hover:bg-surface-muted"
          type="button"
          aria-label={`Actions for ${rental.name}`}
        >
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-max border-collapse text-sm">
        ...
        <tbody>
          {rows.map((row) => (
            <tr
              key={`${row.rental.vm_id}-${row.rental.stream_id}`}
              className="border-b border-border last:border-b-0"
            >
              <td className="py-4 pr-4 font-mono text-text-primary">
                {shortId(row.rental.stream_id)}
              </td>
              <td className="px-4 py-4 font-medium text-text-primary">
                {row.rental.name}
              </td>
              <td className="px-4 py-4 font-mono">
                {shortId(row.rental.provider_id)}
              </td>
              <td className="px-4 py-4">
                {row.remainingSeconds == null
                  ? "-"
                  : humanDuration(row.remainingSeconds)}
              </td>
              ...
              <td className="py-4 pl-4 text-right">
                <button
                  className="rounded-md p-1 text-text-primary hover:bg-surface-muted"
                  type="button"
                  aria-label={`Actions for stream ${row.rental.stream_id}`}
                >
                  <RiMoreFill className="h-5 w-5" aria-hidden />
                </button>
              </td>
